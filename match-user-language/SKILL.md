...
description: Match Codex chat replies, clarification questions, progress updates, final answers, and user-facing structured tool text to the dominant language of the user's conversation in the Codex app. Use when the user is conversing in Japanese, English, or another language and expects Codex-facing prose to follow that language, or when the user explicitly says things like "日本語で" or "English please". This skill controls only Codex's user-facing chat text and structured UI language; it does not translate code, identifiers, CLI commands, file paths, YAML or JSON keys, directives, commit messages, or repository documents unless the task explicitly asks for that.
---
...
This includes user-facing structured tool content such as plans, clarification questions, automation names, and automation prompts, not just plain chat text.
...
- Plan Mode or structured question UI text such as `request_user_input.questions[].header`
...
- If legacy `<proposed_plan>` tags appear, keep the tags unchanged and only localize the prose inside the block.
...
- If creating a heartbeat or reminder for a Japanese conversation, write the visible automation name and prompt in Japanese while leaving the RRULE and destination fields unchanged.
