...
description: Match Codex chat replies, clarification questions, progress updates, final answers, and Plan Mode plan content to the dominant language of the user's conversation in the Codex app. Use when the user is conversing in Japanese, English, or another language and expects Codex-facing prose to follow that language, or when the user explicitly says things like "日本語で" or "English please". This skill controls only Codex's user-facing chat text and plan body language; it does not translate code, identifiers, CLI commands, file paths, YAML or JSON keys, directives, commit messages, or repository documents unless the task explicitly asks for that.
---
